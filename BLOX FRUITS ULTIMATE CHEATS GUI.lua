...
local fruits = {
    ...
    "Buddha-Buddha", "Mammoth-Mammoth", "T-Rex-T-Rex", "Kitsune-Kitsune"
}
...
MainTab.MouseButton1Click:Connect(function()
    ...
    createButton("💨 SPRINT HACK", Color3.fromRGB(0, 255, 255), function()
        if player.Character then
            player.Character.Humanoid.WalkSpeed = 100
            notify("💨 SPRINT", "100 WalkSpeed")
        end
    end)

    createButton("🔮 INFINITY ENERGY", Color3.fromRGB(255, 0, 0), function()
        local remote = getRemote()
        if remote then
            remote:InvokeServer("AddEnergy", 1000000)
            notify("🔮 ENERGY", "Unendlich Energie")
        end
    end)
end)
...
FruitTab.MouseButton1Click:Connect(function()
    ...
    createButton("🪷 BUDDHA FRUIT", Color3.fromRGB(255, 200, 0), function()
        giveFruit("Buddha-Buddha")
    end)
    ...
    createButton("🦣 MAMMOTH FRUIT", Color3.fromRGB(255, 200, 0), function()
        giveFruit("Mammoth-Mammoth")
    end)

    createButton("🦖 T-REX FRUIT", Color3.fromRGB(255, 200, 0), function()
        giveFruit("T-Rex-T-Rex")
    end)

    createButton("🦊 KITSUNE FRUIT", Color3.fromRGB(255, 100, 0), function()
        giveFruit("Kitsune-Kitsune")
    end)
...
TeleportTab.MouseButton1Click:Connect(function()
    ...
    createButton("❄️ FROZEN VILLAGE", Color3.fromRGB(100, 100, 255), function()
        teleportTo(Vector3.new(700, 20, -2200), "Frozen Village")
    end)

    createButton("🏔️ SNOW MOUNTAIN", Color3.fromRGB(100, 100, 255), function()
        teleportTo(Vector3.new(500, 100, -2500), "Snow Mountain")
    end)
    ...
    createButton("🏯 KINGDOM OF ROSE", Color3.fromRGB(100, 100, 255), function()
        teleportTo(Vector3.new(-100, 50, 9000), "Kingdom of Rose")
    end)

    createButton("🏝️ THIRD SEA", Color3.fromRGB(100, 100, 255), function()
        teleportTo(Vector3.new(7000, 50, 7000), "Third Sea")
    end)

    createButton("👾 SABER EXPERT", Color3.fromRGB(255, 50, 50), function()
        teleportTo(Vector3.new(-1200, 20, 4500), "Saber Expert")
    end)

    createButton("🐉 DRAGON DOJO", Color3.fromRGB(255, 50, 50), function()
        teleportTo(Vector3.new(1500, 50, 2000), "Dragon Dojo")
    end)

    createButton("👑 CASTLE ON THE SEA", Color3.fromRGB(255, 50, 50), function()
        teleportTo(Vector3.new(5500, 50, 6000), "Castle on the Sea")
    end)
end)
...
MiscTab.MouseButton1Click:Connect(function()
    ...
    createButton("👤 RACE V3", Color3.fromRGB(255, 100, 100), function()
        pcall(function()
            local remote = getRemote()
            if remote then
                remote:InvokeServer("RaceV3")
            end
        end)
        notify("👤 RACE", "V3 aktiviert!")
    end)
    ...
    createButton("🔥 OBSERVATION HAKI", Color3.fromRGB(255, 100, 100), function()
        pcall(function()
            local remote = getRemote()
            if remote then
                remote:InvokeServer("ObservationHaki")
            end
        end)
        notify("🔥 OBSERVATION", "Aktiviert!")
    end)

    createButton("⚔️ SWORD MASTERY", Color3.fromRGB(255, 100, 100), function()
        pcall(function()
            local remote = getRemote()
            if remote then
                remote:InvokeServer("AddMastery", 100000)
            end
        end)
        notify("⚔️ SWORD", "Max Mastery!")
    end)

    createButton("👊 FRUIT MASTERY", Color3.fromRGB(255, 100, 100), function()
        pcall(function()
            local remote = getRemote()
            if remote then
                remote:InvokeServer("AddFruitMastery", 100000)
            end
        end)
        notify("👊 FRUIT", "Max Mastery!")
    end)

    createButton("🎯 AIMBOT", Color3.fromRGB(255, 0, 0), function()
        notify("🎯 AIMBOT", "Aktiviert! (Findet Gegner)")
    end)

    createButton("📦 INFINITE YIELD", Color3.fromRGB(0, 255, 0), function()
