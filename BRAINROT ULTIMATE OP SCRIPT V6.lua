...
-- BESTIMMTE PETS SUCHEN UND BRINGEN
function findAndBringPet(petName)
    local count = 0
    for _, obj in pairs(Workspace:GetDescendants()) do
        if obj.Name:lower():find(petName:lower()) then
            if obj:IsA("Model") and obj:FindFirstChild("HumanoidRootPart") then
                pcall(function()
                    obj.HumanoidRootPart.CFrame = rootPart.CFrame * CFrame.new(0, 0, 3)
                    count = count + 1
                end)
            elseif obj:IsA("Tool") and obj:FindFirstChild("Handle") then
                pcall(function()
                    obj.Handle.CFrame = rootPart.CFrame * CFrame.new(0, 0, 3)
                    count = count + 1
                end)
            end
        end
    end
    notification("🔍 " .. count .. " " .. petName .. " gefunden und gebracht!", "success")
end
...
function showExtraTab()
    ...
    y = createToggle("👤 AUTO STEAL", y, Color3.fromRGB(255, 0, 0), function() return autoSteal end, startAutoSteal)
    y = createToggle("🖱️ AUTO CLICKER", y, Color3.fromRGB(0, 255, 0), function() return clickerActive end, startAutoClicker)
    y = createToggle("🌀 ANTI-AFK", y, Color3.fromRGB(0, 255, 255), function() return antiAFK end, function()
        antiAFK = not antiAFK
        if antiAFK then
