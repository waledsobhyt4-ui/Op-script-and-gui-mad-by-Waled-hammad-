...
local colors = {
    ...
    rubox = Color3.fromRGB(255, 200, 0), -- Gold für RUBOX
...
mainFrame.Size = UDim2.new(0, 450, 0, 650) -- Größer für mehr Buttons
...
scanBtn.Size = UDim2.new(0.5, -6, 0, 45)
...
getBtn.Size = UDim2.new(0.5, -6, 0, 45)
...
scanRuboxBtn.Size = UDim2.new(0.5, -6, 0, 45)
scanRuboxBtn.Position = UDim2.new(0, 0, 0, 90)
...
resetBtn.Position = UDim2.new(0, 0, 1, -45)
...
local function addToolToList(toolName, toolInstance, isRubox)
    ...
    toolButton.Size = UDim2.new(1, -10, 0, 35)
    ...
    toolButton.MouseEnter:Connect(function()
        tweenService:Create(toolButton, TweenInfo.new(0.2), {BackgroundTransparency = 0}):Play()
    end)
    toolButton.MouseLeave:Connect(function()
        tweenService:Create(toolButton, TweenInfo.new(0.2), {BackgroundTransparency = isRubox and 0.2 or 0.5}):Play()
    end)
end
...
local function scanForRuboxTools()
    ...
    -- Suche nach Tools mit "RUBOX" im Namen
    local allTools = game:GetDescendants()
    ...
    for _, obj in ipairs(allTools) do
        if obj:IsA("Tool") and obj.Parent then
            ...
            if string.find(toolName:upper(), "RUBOX") or 
               ...
               string.find(toolName:upper(), "GOLD") then
...
minimizeBtn.MouseButton1Click:Connect(function()
    ...
    if isMinimized then
        ...
        minimizeBtn.Text = "□"
        setStatus("📱 GUI minimiert - Klicke □ zum öffnen", nil)
    else
        -- Maximieren
        contentFrame.Visible = true
        mainFrame.Size = originalSize
        minimizeBtn.Text = "−"
        setStatus("🟢 GUI geöffnet", nil)
    end
end)
...
print("✅ OP ToolGiver mit RUBOX Support geladen! Made by Waled-hammad")
setStatus("🟢 Bereit - RUBOX Support aktiv", true)
