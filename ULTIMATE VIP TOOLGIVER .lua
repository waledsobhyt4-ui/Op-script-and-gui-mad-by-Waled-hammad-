-- ============================================
-- ULTIMATE VIP TOOLGIVER - ROBUX TOOLS
-- SCANNT ALLE VERFÜGBAREN TOOLS IM SPIEL
-- AUTO VIP FÜR JEDEN USER | KEIN FAKE
...
-- VIP Status Anzeige
local function showVIPStatus()
    return "👑 VIP AKTIV (User: " .. userID .. ")"
end

-- ============================================
-- ROBUX TOOLS ERKENNUNG
-- ============================================
local robuxTools = {}
local scannedTools = {}
local rareTools = {}
...
-- Marketplace Produkte scannen
function scanDevProducts()
    devProducts = {}
    for _, obj in pairs(ReplicatedStorage:GetDescendants()) do
        if obj:IsA("DevProduct") or obj:IsA("Product") or obj.Name:lower():find("product") then
            table.insert(devProducts, obj)
        end
    end
end
...
VIPStatus.Size = UDim2.new(0.5, 0, 0, 35)
VIPStatus.Position = UDim2.new(0.5, -100, 0, 8)
VIPStatus.BackgroundColor3 = Color3.fromRGB(255, 215, 0)
VIPStatus.Text = showVIPStatus()
...
function scanAllTools()
    ...
    for _, obj in pairs(Workspace:GetDescendants()) do
        if obj:IsA("Tool") then
            ...
            -- ROBUX Tools erkennen (kostenpflichtige Items)
            if obj.Name:lower():find("robux") or obj.Name:lower():find("premium") or 
               obj.Name:lower():find("gamepass") or obj.Name:lower():find("vip") or
               obj:FindFirstChild("ProductId") or obj:FindFirstChild("PriceInRobux") then
                table.insert(robuxTools, obj)
                robuxCount = robuxCount + 1
            end
            
            -- Seltene Tools erkennen
            if obj.Name:lower():find("legend") or obj.Name:lower():find("mythic") or 
               obj.Name:lower():find("rare") or obj.Name:lower():find("epic") or
               obj.Name:lower():find("god") or obj.Name:lower():find("admin") then
    ...
    -- Info anzeigen
    local infoBtn = createButton("📊 GEFUNDEN: " .. count .. " TOOLS", y, Color3.fromRGB(0, 255, 255), function() end)
    ...
    -- Buttons zum Einsammeln
    if count > 0 then
        local getAllBtn = createButton("🎁 ALLE TOOLS EINSAMMELN", y, Color3.fromRGB(0, 255, 0), function()
            giveAllTools()
        end)
        y = y + 55
        
        if robuxCount > 0 then
            local getRobuxBtn = createButton("💰 ROBUX TOOLS EINSAMMELN", y, Color3.fromRGB(255, 215, 0), function()
                giveRobuxTools()
            end)
            y = y + 55
        end
        ...
        if rareCount > 0 then
            local getRareBtn = createButton("⭐ SELTENE TOOLS EINSAMMELN", y, Color3.fromRGB(255, 0, 255), function()
                giveRareTools()
            end)
            y = y + 55
        end
...
function giveRobuxTools()
    ...
    notification("💰 " .. count .. " ROBUX Tools erhalten! (Kostenlos)")
end
...
function giveRandomRobux()
    if #robuxTools > 0 then
        ...
        local clone = randomTool:Clone()
        clone.Parent = player.Backpack
        notification("💰 " .. randomTool.Name .. " (ROBUX) erhalten!")
    else
...
function giveRandomRare()
    if #rareTools > 0 then
        ...
        local clone = randomTool:Clone()
        clone.Parent = player.Backpack
        notification("⭐ " .. randomTool.Name .. " (SELTEN) erhalten!")
    else
...
function freezeTools()
    ...
    notification("📦 Tools eingefroren (nicht verlierbar)!")
end
...
function showRobux()
    ...
    if #robuxTools > 0 then
        ...
        local noRobux = createButton("❌ Keine ROBUX Tools gefunden", y, Color3.fromRGB(255, 0, 0), function()
            scanAllTools()
        end)
...
function showAll()
    ...
    if #scannedTools > 0 then
        ...
        local noTools = createButton("🔍 ZUERST SCANNEN", y, Color3.fromRGB(255, 165, 0), function()
            scanAllTools()
        end)
...
function showRare()
    ...
    if #rareTools > 0 then
        ...
        local noRare = createButton("❌ Keine seltenen Tools", y, Color3.fromRGB(255, 0, 0), function()
            scanAllTools()
        end)
...
function showVIP()
    ...
    y = createButton("🎲 RANDOM ROBUX TOOL", y, Color3.fromRGB(255, 215, 0), giveRandomRobux)
    y = createButton("🎲 RANDOM SELTENES", y, Color3.fromRGB(255, 0, 255), giveRandomRare)
    y = createButton("🎲 RANDOM TOOL", y, Color3.fromRGB(255, 165, 0), giveRandomTool)
